...
router.get('/memories/pending', authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        const profile = await db.findProfileByUserId(userId);
        if (!profile)
            return res.json({ facts: [], questions: [] });
        const memories = await db.getMemories(profile.id);
        const facts = memories.filter((m) => m.type === 'LEARNED_FROM_GUEST');
        const questions = memories.filter((m) => m.type === 'GUEST_QUESTION');
...
router.post('/memories/:id/approve', authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
...
router.delete('/memories/:id', authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
