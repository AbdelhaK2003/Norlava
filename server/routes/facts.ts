...
router.get('/pending-facts', authenticateToken, async (req: AuthRequest, res) => {
    try {
...
router.post('/approve-fact', authenticateToken, async (req: AuthRequest, res) => {
    try {
        ...
        if (!updatedAiContext.includes('FACTS I\'VE LEARNED:')) {
            ...
            updatedAiContext = updatedAiContext.replace(
                /FACTS I'VE LEARNED FROM VISITORS:[\s\S]*?(?=\n\n|$)/,
                `FACTS I'VE LEARNED FROM VISITORS:\n${learnedFacts.map((fact: string) => `• ${fact}`).join('\n')}`
            );
        }
...
router.post('/reject-fact', authenticateToken, async (req: AuthRequest, res) => {
    try {
        const { factIndex } = req.body;
